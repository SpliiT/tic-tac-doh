...
function verifierGagnant() {
  ...
  function ligneGagnante(ligne) {
    ...
    return (
      symbole !== "" &&
      ligne.every((indiceCase) => etatJeu[indiceCase] === symbole)
    );
  }

  // Vérification des lignes, colonnes et diagonales pour déterminer s'il y a un gagnant
  return (
    lignes.some(ligneGagnante) ||
    colonnes.some(ligneGagnante) ||
    diagonales.some(ligneGagnante)
  );
}
...
function trouverCoupBloquant() {
  for (let i = 0; i < etatJeu.length; i++) {
    if (
      etatJeu[i] === "" &&
      estCoupGagnant(i, joueurCourant === "saxo" ? "skate" : "saxo")
    ) {
      return i;
    }
  }
